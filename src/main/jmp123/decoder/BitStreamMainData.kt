 ...
 */
class BitStreamMainData(len: Int, extra: Int) : BitStream(len, extra) {
    // 用于哈夫曼解码的大值区
    private lateinit var lin // lin[0..15]=0
            : IntArray
    private lateinit var htbv: Array<ShortArray?>
    private lateinit var region // 某一码表解码用于解码主数据的区域
            : IntArray

    // 用于解码小值区
    private lateinit var htc0: ShortArray
    private lateinit var htc1: ShortArray
     ...
     */
    fun decodeHuff(ci: Layer3.ChannelInformation, hv: IntArray): Int {
        var tmp: Int
        var linbits: Int
        var maxidx: Int
        var idx = 0
		 ...
		 */
        var part3len: Int = ci.part2_3_length - ci.part2_length
        var x: Int = ci.region1Start // region1
        var y: Int = ci.region2Start // region2
        var i: Int = ci.big_values shl 1 // bv
		 ...
		 * 2. 使位流缓冲区按字节对齐
		 */
        var num = 8 - bitPos and 7
        var mask = 0
        if (num > 0) {
            mask = getBits9(num)
            mask = mask shl 32 - num
            part3len -= num
        }
        ...
        while (i < 3) {
            ...
            while (idx < maxidx) {
                ...
                while (num < 24) { // refresh mask
                    mask = mask or (b[bytePos++].toInt() and 0xff shl 24 - num)
                    num += 8
                    part3len -= 8
                }
                tmp = mask
                y = htab!![tmp ushr 30].toInt()
                while (y < 0) {
                    tmp = tmp shl 2
                    y = htab[(tmp ushr 30) - y].toInt()
                }
                x = y shr 8 // x暂存hlen
                num -= x
                mask = mask shl x
                x = y shr 4 and 0xf // 解得x,y
                y = y and 0xf
                if (x != 0) {
                    if (x == 15 && linbits != 0) {
                        while (num < 24) { // refresh mask
                        ...
                        x += mask ushr 32 - linbits // 循环右移
                        num -= linbits
                        mask = mask shl linbits
                    }
                    hv[idx++] = if (mask < 0) -x else x
                    num--
                    mask = mask shl 1
                } else hv[idx++] = 0
                if (y != 0) {
                    if (y == 15 && linbits != 0) {
                        ...
                        y += mask ushr 32 - linbits
                        num -= linbits
                        mask = mask shl linbits
                    }
                    hv[idx++] = if (mask < 0) -y else y
                    num--
                    mask = mask shl 1
                } else hv[idx++] = 0
            }
        ...
        /*
		 * 4. 解码count1区
		 */htab = if (ci.count1table_select === 0) htc0 else htc1
        while (idx < 572) {
            ...
            tmp = mask
            y = htab[tmp ushr 28].toInt()
            while (y < 0) {
                tmp = tmp shl 4
                y = htab[(tmp ushr 28) - y].toInt()
            }
            x = y shr 8 // hlen
            mask = mask shl x
            num -= x
            ...
            // 一个码字(hcod)解码得到4个值
            if (28.let { y = y shl it; y } < 0) {
                hv[idx++] = if (mask < 0) -1 else 1
                num--
                mask = mask shl 1
            } else hv[idx++] = 0
            if (1.let { y = y shl it; y } < 0) {
                hv[idx++] = if (mask < 0) -1 else 1
                num--
                mask = mask shl 1
            } else hv[idx++] = 0
    ...
    //-------------------------------------------------------------------------
    private fun initHuffTable() {
        // 码表 htbvx[]: 16个数组共2752元素
        val htbv0 = shortArrayOf(0, 0, 0, 0)
        val htbv1 = shortArrayOf(-4, 528, 256, 256, 785, 785, 769, 769)
        val htbv2 = shortArrayOf( //20
                -4, -16, 256, 256, -8, -12, 785, 785, 1570, 1538, 1298, 1298, 1313, 1313, 1312, 1312,
                769, 769, 784, 784)
        val htbv3 = shortArrayOf( //16
                -4, 529, 513, 512, -8, -12, 784, 784, 1570, 1538, 1298, 1298, 1313, 1313, 1312, 1312)
        val htbv5 = shortArrayOf( //32
